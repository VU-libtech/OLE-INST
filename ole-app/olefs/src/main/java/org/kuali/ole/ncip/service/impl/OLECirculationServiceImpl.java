...
public class OLECirculationServiceImpl implements OLECirculationService {
        ...
        if (!loanProcessor.hasCirculationDesk(operator)) {
            ...
            if (olePatronDocumentList.size() > 0) {
                ...
                try {
                    ...
                    List<OLECheckedOutItem> oleCheckedOutItemList = getPatronCheckedOutItemList(oleLoanDocumentList,olePatronDocument.getOleBorrowerType().getBorrowerTypeCode());
        ...
        if (!loanProcessor.hasCirculationDesk(operator)) {
            ...
            if (olePatronDocumentList.size() > 0) {
                ...
                if (oleLoanDocumentList != null && oleLoanDocumentList.size() > 0) {
                    List<OLECheckedOutItem> oleCheckedOutItemList = getPatronCheckedOutItemList(oleLoanDocumentList,patronType);
                    oleCheckedOutItems.setCheckedOutItems(oleCheckedOutItemList);
    ...
    public List<OLEHold> getHoldsList(List<OleDeliverRequestBo> oleDeliverRequestBoList) throws Exception {
        ...
        for (OleDeliverRequestBo oleDeliverRequestBo : oleDeliverRequestBoList) {
            ...
            Map<String, String> requestTypeMap = new HashMap<String, String>();
            requestTypeMap.put(OLEConstants.OleDeliverRequest.REQUEST_TYPE_ID, oleDeliverRequestBo.getRequestTypeId());
            List<OleDeliverRequestType> oleDeliverRequestTypeList = (List<OleDeliverRequestType>) KRADServiceLocator.getBusinessObjectService().findMatching(OleDeliverRequestType.class, requestTypeMap);
            if (oleDeliverRequestTypeList.size() > 0) {
                oleHold.setRequestType(oleDeliverRequestTypeList.get(0).getRequestTypeCode());
                ...
                if (oleLoanDocumentList.get(0).getLoanDueDate() != null) {
                    ...
                    if (oleDeliverRequestBo.getPickUpLocationId() != null) {
                        ...
                        if (oleCirculationDeskList.size() > 0) {
                            ...
                            oleHold.setDateAvailableExpires(addDate(new java.sql.Date(oleLoanDocumentList.get(0).getLoanDueDate().getTime()), Integer.parseInt(oleCirculationDeskList.get(0).getOnHoldDays())).toString());
                        }
                    }
                } else {
     ...
     * @return
     */
    public List<OLECheckedOutItem> getPatronCheckedOutItemList(List<OleLoanDocument> oleLoanDocumentList,String patronType){
        SimpleDateFormat fmt = new SimpleDateFormat(OLEConstants.OleDeliverRequest.DATE_FORMAT);
