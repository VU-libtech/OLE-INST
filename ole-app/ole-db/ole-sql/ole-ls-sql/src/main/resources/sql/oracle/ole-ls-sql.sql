...
(SELECT P.OLE_PTRN_ID, P.BARCODE AS PTRN_BRCD, (SELECT COUNT(*) FROM OLE_DLVR_PTRN_BILL_T PB WHERE P.OLE_PTRN_ID = PB.OLE_PTRN_ID) AS BILL_CNT, (SELECT COUNT(*) FROM OLE_DLVR_LOAN_T PLN WHERE P.OLE_PTRN_ID = PLN.OLE_PTRN_ID) AS LOAN_CNT, (SELECT COUNT(*) FROM OLE_DLVR_RQST_T PRQST WHERE P.OLE_PTRN_ID = PRQST.OLE_PTRN_ID) AS RQST_CNT, (SELECT COUNT(*) FROM OLE_DLVR_TEMP_CIRC_RECORD PTMPHSTY WHERE P.OLE_PTRN_ID = PTMPHSTY.OLE_PTRN_ID) AS TMP_CIR_HSTY_CNT, (SELECT DLVR_BORR_TYP_NM FROM OLE_DLVR_BORR_TYP_T WHERE DLVR_BORR_TYP_ID=P.BORR_TYP) AS PTRN_TYP, P.BORR_TYP AS PTRN_TYP_ID, P.EXPIRATION_DATE AS EXPR_DT, ENT.ENTITY_ID, ENT_NM.FIRST_NM, ENT_NM.MIDDLE_NM, ENT_NM.LAST_NM, ENT_NM.SUFFIX_NM, ENT_NM.PREFIX_NM, (SELECT ENT_EM.EMAIL_ADDR FROM KRIM_ENTITY_EMAIL_T ENT_EM WHERE ENT.ENTITY_ID = ENT_EM.ENTITY_ID AND DFLT_IND='Y') AS EMAIL, (SELECT ENT_PH.PHONE_NBR FROM KRIM_ENTITY_PHONE_T ENT_PH WHERE ENT.ENTITY_ID = ENT_PH.ENTITY_ID AND DFLT_IND='Y') AS PHONE, concat(concat(ENT_NM.LAST_NM,', '),ENT_NM.FIRST_NM) AS NAME, P.ACTV_IND AS ACTV_IND FROM OLE_PTRN_T P, KRIM_ENTITY_T ENT, KRIM_ENTITY_NM_T ENT_NM WHERE P.OLE_PTRN_ID = ENT.ENTITY_ID AND ENT.ENTITY_ID = ENT_NM.ENTITY_ID)
